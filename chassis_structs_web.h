...
struct backSingleBoardQuery                         /*定义机箱管理发往web网管单板维护查询指令帧格式*/
{
  unsigned short frameHead;                         //帧头
  unsigned short frameCnt;                          //帧计数
  unsigned char frameType;                          //报文类型
...
struct backHealthQuery             /*定义机箱管理返回WEB网管健康管理查询指令帧格式*/
{
  ...
  unsigned short frameCnt;                    //帧计数
  unsigned char frameType;                    //报文类型
  struct healthManagement healthAttribute[17];//所有板卡健康属性
  unsigned short backup1;                     //备份1
  unsigned short backup2;                     //备份2
  unsigned short frameEnd;                    //帧尾
};            						          //back_health_query
 ...
 * 网管软件 -------------> 机箱管理软件 结构体定义
 *********************************************************************/
struct receiveSingleBoardQuery       /*定义接收web设置数据帧格式和查询指令帧格式*/
{
  unsigned short frameHead;              //帧头
  unsigned short frameCnt;               //帧计数
  ...
  unsigned short backup;                 //备份
  unsigned short frameEnd;               //帧尾
};                                       //receive_single_query

struct FanState            /*定义接收Web风扇管理查询指令帧格式*/
{
  unsigned short frameHead;              //帧头
  unsigned short frameCnt;               //帧计数
  unsigned char slot;                    //固定槽位A2
...
struct receiveHealthManagement          /*定义接收Web健康管理阈值设置指令和返回阈值数据帧格式*/
{
  unsigned short frameHead;             //帧头
  unsigned short frameCnt;              //帧计数
  unsigned char frameType;              //报文类型
  ...
  unsigned short frameEnd;              //帧尾
};                                      //receive_health_management      back_threshold_data
